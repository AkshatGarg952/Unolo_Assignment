...
router.get('/stats', authenticateToken, requireManager, async (req, res) => {
    try {
        // BUG 3.2: Fixed Timezone Data Discrepancy
        // Incorrect Code (Uses Server Time/UTC):
        // const today = new Date().toISOString().split('T')[0];

        // Correct Code: Use IST (Indian Standard Time)
        ...
        const [todayCheckins] = await pool.execute(
            `SELECT ch.*, u.name as employee_name, c.name as client_name
             ...
             WHERE u.manager_id = ? AND DATE(datetime(ch.checkin_time, '+05:30')) = ?
    ...
    try {
        ...
        const [todayCheckins] = await pool.execute(
            `SELECT ch.*, c.name as client_name
             FROM checkins ch
             INNER JOIN clients c ON ch.client_id = c.id
             WHERE ch.employee_id = ? AND DATE(datetime(ch.checkin_time, '+05:30')) = ?
             ORDER BY ch.checkin_time DESC`,
            [req.user.id, today]
        );
        ...
        // BUG 1.4: Fixed SQLite Incompatibility
        // Incorrect Code (MySQL specific functions):
