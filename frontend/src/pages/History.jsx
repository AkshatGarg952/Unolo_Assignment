...
function History({ user }) {
    ...
    // BUG 3.5: Fixed Performance Issue
    // Incorrect Code (Recalculates on every render):
    // const totalHours = (checkins || []).reduce((total, checkin) => {
    //     ...
    // }, 0);

    // Correct Code: Use useMemo to memoize calculation
                                    ...
                                    <tr key={checkin.id} className="border-t hover:bg-gray-50">
                                        ...
                                        <td className="px-4 py-3">
                                            {checkin.distance_from_client !== null
                                                ? <span className={checkin.distance_from_client > 0.5 ? 'text-yellow-600 font-medium' : ''}>
